# <center>Fork from [theniceboy](https://github.com/theniceboy/nvim)</center>
...
+ [安装此配置后你需要做的事](#安装此配置后你需要做的事)
	...
	* [快捷键](#快捷键)
		- [1 基础编辑器特性](#1-基础编辑器特性)
			...
			+ [1.3 改变了在普通模式、可是模式、操作符等待模式下的操作键为](#13-改变了在普通模式可是模式操作符等待模式下的操作键为)
			+ [1.4 改变了在普通模式下的操作键位](#14-改变了在普通模式下的操作键位)
			+ [1.5 改变了插入模式的键位](#15-改变了插入模式的键位)
			+ [1.6 改变了在可视模式下的键位](#16-改变了在可视模式下的键位)
			+ [1.6 改变了在命令行模式下的键位](#16-改变了在命令行模式下的键位)
		- [2 窗口管理](#2-窗口管理)
...
## 首先
- [ ] 执行 `:checkhealth` 根据建议安装需要的模块


## 安装此配置后你可能要想做的事
- [ ] 安装 `python`, 使用`pip`, 然后在终端输入 `pip install pynvim` 建议python2，3都安装
- [ ] 安装 `nodejs`, 然后在终端输入 `npm install -g neovim` 或者`yarn global add neovim`
- [ ] 安装 `ruby`, 然后在终端输入 `gem install neovim`
...
### Ruby
- [ ] 在init.vim里添加ruby路径 `let g:ruby_host_prog = '/usr/bin/ruby'`
...
#### 1.3 改变了在普通模式、可是模式、操作符等待模式下的操作键为
| 快捷键          | 行为                             |
|-----------------|----------------------------------|
| `Ctrl` `q`      | 退出vim                          |
| `space` `r` `c` | 跳转到nvim配置文件               |
| `l`             | **撤销**                         |
| `space` `enter` | 取消搜索后高亮                   |
| `SPACE` `d` `w` | 寻找近处的重复单词               |
| `SPACE` `o`     | 折叠代码                         |
| `\` `g`         | Git                              |
| `Ctrl` `g`      | 在vim终端下打开lazygit           |
| `\` `v`         | 当前光标处选择到行尾             |
| `-`             | 下一个搜索                       |
| `=`             | 上一个搜索                       |
| `s`             | 无操作                           |
